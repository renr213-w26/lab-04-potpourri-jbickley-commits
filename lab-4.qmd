...
Chunk Label 1: This is not a proper chunk label and will cause an error in rendering, you cannot add a line break as it sees that as a separate instruction. It could be; #\| label: a-label-with-a-line-break to correct it.

Chunk Label 2: This label is correct, however, not best practice because it is so long and is just not very practical as a label. They could rename to something like #\| label: a-really-long-label

Chunk Label 3: This label will produce an error because of the spaces. It should be re-written as #\| label: label-with-spaces

Chunk Label 4: This label is both usable and practical. It uses dashes and does render properly.

b.  Which of the chunk label options above is the best option? Explain your reasoning in 1-2 sentences.

Chunk Label 4 is the clear winner, it is easily readable and concise and it will render. It follows the recommended practices.
...
Echo controls if the code chunk is actually shown in the code document, if its set to false the code won't be shown in the rendered document but the output still is.

Warning controls if the warnings show up in the rendered output. This chunk produces a warning that a couple rows are removed for missing or out of range values, if Warning was set to false it would be hidden in the output.

eval controls if the code even runs at all. If it is set to false you can still see the code (as long as echo is not false) but it does not actually run.
...
Fig-width sets the width of the graph output, with higher numbers resulting in larger widths.

Fig-asp sets the length of the graph output, however not directly, it sets using a ratio of fig-width so if it were 0.5 the height would be half of the length
...
There is an error in the code chunk (gplot instead of ggplot), what error: false does is force the document to render even though there is an error. With true it would get an error before producing the rendered output.
...
a.  What is misleading about this graph?
    ...
    This graph is highly misleading because the proportions don't align with the size of the actual parts, for example for energy the public sector portion is 87%, but only takes up around 50% of the bar chart. This also makes comparing and understanding very difficult.
...
b.  Suppose you wanted to recreate this plot, with improvements to avoid its misleading pitfalls from part (a). You would obviously need the data from the survey in order to be able to do that. How many observations would this data have? How many variables (at least) should it have, and what should those variables be?
    ...
    There are1858 respondents, but four categories, so there would be 7432 individual data points. The least amount of varialbes is likely having 5 columns, one for some kind of id or point number, and one for each response (Rail, Water, Energy, Royal Mail). Though they would have to pivot longer probably to make a graph like this in R.
...
c.  Load the data for this survey from `data/survation.csv`. Confirm that the data match the percentages from the visualization. That is, calculate the percentages of public sector, private sector, don't know for each of the services and check that they match the percentages from the plot.
    ...
    # load data --- I CANNOT FIND THE DATASET ---
    survation_pct <- tribble(
      ~service,      ~response,          ~percent,
      "Rail",        "Public sector",    94,
      "Rail",        "Private sector",    3,
      "Rail",        "Don't know",        3,
      
      "Water",       "Public sector",    94,
      "Water",       "Private sector",    3,
      "Water",       "Don't know",        3,
      
      "Energy",      "Public sector",    87,
      "Energy",      "Private sector",    6,
      "Energy",      "Don't know",        7,
      
      "Royal Mail",  "Public sector",    85,
      "Royal Mail",  "Private sector",    7,
      "Royal Mail",  "Don't know",        8
    )
    survation_pct
    ```
...
survation_pct |> mutate(service = factor(service, levels = c("Rail", "Water", "Energy", "Royal Mail"))) |> ggplot(aes(percent, service, fill = response ))+ geom_col() +
scale_fill_manual(values = c("Public sector" = "#0B78A5", "Private sector" = "#F05A28", "Don't know" = "gray60"))+
theme(legend.position = "bottom")+
labs(title = "Do you Think the Following Services Should be Run in the \nPrivate Sector or the Public Sector?",
caption = "All Respondents Unweighted Total = 1858",
  x = "",
  y= "")
```
...
This plot is must more proportionate to the actual response values and allows the user to easily understand the survey results.
...
```{r}
datasaurus_dozen |> group_by(dataset) |> summarise(mean_x = mean(x), mean_y = mean(y), std_x = sd(x), std_y = sd(y), correlation = cor(x,y)) |> print(n = 13)
```

Across all of the summary statistics, the data sets are extremely similar between each other the means, standard deviations,9 and correlations are all within margin of error across data sets.
...
Though all the data sets have very similar summary statistics, they are clearly completely different and the distribution of the data is not very similar at all. It highlights how looking at summary statistics can be misleading and why investigating data further is necessary.
